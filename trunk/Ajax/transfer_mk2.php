...
function getDataMataKuliahTRMahasiswa($kode_mk){
		$sql = "SELECT * FROM t_mata_kuliah AS m JOIN tr_mata_kuliah_mahasiswa AS j ON m.kode_mata_kuliah=j.kode_mata_kuliah JOIN t_mahasiswa as t ON t.nrp=j.nrp WHERE m.kode_mata_kuliah = '$kode_mk'";
		$rs = mysql_query($sql);
		$data;$k=0;
		while($row = mysql_fetch_array($rs)){
			$data[$k]['nrp'] = $row['nrp'];	
...
function getTabelDatabaseMataKuliah_Syarat($search,$searchin,$orderby,$order,$color){
			$s = "Ordering Table with Coloum <b>'".$orderby."'</b> By <b>'".$order."'</b>";
			$sql = "SELECT * FROM tr_mata_kuliah_syarat as mks JOIN t_mata_kuliah as mk ON mks.kode_mata_kuliah = mk.kode_mata_kuliah ORDER BY mks.".$orderby." ".$order;
			if($search!=""){
				$s = "Searching <b>'".$search."'</b> In Table with Coloum <b>'".$searchin."'</b> and ".$s;
				$sql = "SELECT * FROM tr_mata_kuliah_syarat as mks JOIN t_mata_kuliah as mk ON mks.kode_mata_kuliah = mk.kode_mata_kuliah WHERE mks.".$searchin." LIKE '%".$search."%' ORDER BY mks.".$orderby." ".$order;
			}
			$s = '<div>'.$s;
			$rs = mysql_query($sql);
			...
			while($row = mysql_fetch_array($rs)){
				...
				.'<a id="v" onclick="javascript:edit_db_mk(3,\''.$kode_mk.'\')" href="#!">&nbsp;</a><a href="#!"><a id="e" onclick="javascript:edit_db_mk(2,\''.$kode_mk.'\')" href="#!">&nbsp;</a><a href="#!"></td>'
				.'</tr>';
				$k++;
			}
			$mk = $mk.'</table>';
...
function getTabelDatabaseMataKuliah_Mahasiswa($search,$searchin,$orderby,$order,$color){
			$s = "Ordering Table with Coloum <b>'".$orderby."'</b> By <b>'".$order."'</b>";
			...
			$rs = mysql_query($sql);
			$mk = '<table id="db" class="'.$color.'">
			<tr id="header_table"><th>subject code</th><th width="40%">subject name</th><th>student nrp</th><th width="40%">student name</th><th>semester</th><th>moment</th><th>date register</th><th>score</th><th>V</th><th>ActION</tr></tr>
			';
...
function getTabelDatabaseMataKuliah_Jurusan($search,$searchin,$orderby,$order,$color){
			$s = "Ordering Table with Coloum <b>'".$orderby."'</b> By <b>'".$order."'</b>";
...
function getTabelDatabaseMataKuliah_Dosen($search,$searchin,$orderby,$order,$color){
			$s = "Ordering Table with Coloum <b>'".$orderby."'</b> By <b>'".$order."'</b>";
...
function getTabelDatabaseMataKuliah($search,$searchin,$orderby,$order,$color){
			$s = "Ordering Table with Coloum <b>'".$orderby."'</b> By <b>'".$order."'</b>";
...
//				$datafj = getFakultasJurusan($row['kode_mata_kuliah']);
				$kode_mk = $row['kode_mata_kuliah'];
				$probis; if($row['probis']=="0"){$probis="No";}else{$probis="Yes";}
				$mk = $mk
				.'<tr'.$tr.' height="20px"><td id="center"><b>'
