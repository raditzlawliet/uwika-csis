...
switch($code){
	case 'db_h' : { //db awal
			$out = '
				<center><h1 style="letter-spacing:20px;"><b>HOME</b></h1><p>
				'.$code.' '.$uid.' '.$admin.' h
				</center>
	...
	case 'db_m' : { //db mahasiswa
			...
			$out = '
				...
				<div id="db_cmd"><a id="add" class="submit" onclick="javascript:edit_db_m(1,null);" href="#!">Add New Account</a>
				...
				<div id="db_mahasiswa">
				'.getTabelDatabaseMahasiswa("","","nrp","ASC","red").'
				</div><p>
				</center>
				...
				function search(db){
					...
					if((search_text==null)||(search_text=="")||(search_text==" ")||(search_text=="  ")){
						$.post("Ajax/database_ajax.php",{code:db, search_text:"", search_in:"", sort_text:sort_text, sort_by:sort_by, color:color}, function(data) {
							$("#db_mahasiswa").html(data);
						}); 
					}else{
						$.post("Ajax/database_ajax.php",{code:db, search_text:search_text, search_in:search_in, sort_text:sort_text, sort_by:sort_by, color:color}, function(data) {
							$("#db_mahasiswa").html(data);
						}); 
					}
				}
