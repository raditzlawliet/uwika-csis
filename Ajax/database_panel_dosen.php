...
switch($code){
	case 'db_d' : { //db mahasiswa
			...
			$out = '
				...
				<div id="db_dosen">
				'.getTabelDatabaseDosen("","","nrp","ASC","red").'
				</div><p>
				</center>
				<script>
				...
				function search(db){
					...
					if((search_text==null)||(search_text=="")||(search_text==" ")||(search_text=="  ")){
						...
						$.post("Ajax/database_ajax_dosen.php",{code:db, search_text:"", search_in:"", sort_text:sort_text, sort_by:sort_by, color:color}, function(data) {
							$("#db_dosen").html(data);
						}); 
					}else{
						$("#db_dosen").html(\'<div><center><img src="images/throbber_white.gif"></center></div>\');
						$.post("Ajax/database_ajax_dosen.php",{code:db, search_text:search_text, search_in:search_in, sort_text:sort_text, sort_by:sort_by, color:color}, function(data) {
							$("#db_dosen").html(data);
						}); 
					}
				}
