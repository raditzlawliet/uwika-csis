...
switch($code){
	...
	if($all!="all"){
			$a = "<center><h1> LAPORAN MATA KULIAH</h1></center>";
			$a = $a.getTabelDatabaseListMahasiswaPerMataKuliah($search_text,$search_in,$sort_text,$sort_by,$color,$start,$limit,$show,$search2x,$ab);
			exit($a);
		break;
	}else{
			$data = getAllDataMK();	
			for($i=0;$i<$data['k'];$i++){
				$zxc =$zxc."<center><h1> LAPORAN MATA KULIAH</h1></center>";
