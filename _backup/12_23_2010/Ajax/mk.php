...
/*	switch($sms){
		...
		if($sms>0 && $sms<=8){
			...
			if(!isset($_COOKIE['set_height_mk_list'])){
				$set_height_mk_list = 300;
			}
			$mk = '<div id="red_mozilla" style="width:50%;"><b>Hint!</b> Choose the Mata Kuliah down here, by click it.<br>Default Height Scroll "description of Mata Kuliah" when you click it is <b>350px</b>,<br>
			...
			';
			$mk = $mk.getListMataKuliahPerSemester($kode_jurusan,$sms,$probis);
/*			$mk = '<ul>
			<li>md5nrp : '.$_SESSION['md5nrp'].'</li>
			<li>turn : '.$_SESSION['turn'].'</li>
			<li>admin : '.$_SESSION['admin'].'</li>
			...
			function GoDAFTARKRSMK(kode_mk,uid){
				data = "&uid=" + encodeURI(uid) + "&kode_mk=" + encodeURI(kode_mk);
